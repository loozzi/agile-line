...
from src.models import Project, Label, IssueLabel
from src.models import Milestone, Issue, Resources, Activity, References
...
def delete_workspace(permalink, password):
    ...
    if is_workspace_user(current_user, current_workspace) is False:
        return _response(403, "Không có quyền truy cập")
    current_workspace_user = WorkspaceUser.query.filter(
                        WorkspaceUser.user_id == current_user.id
                            ).filter(
                            WorkspaceUser.workspace_id == current_workspace.id
                            ).first()
    ...
    for i in list_project_workspace:
        # xóa milestone
        list_milestone = Milestone.query.filter_by(
                            project_id=i.id).all()
        for j in list_milestone:
            db.session.delete(j)
            db.session.flush()
        # xóa role và user role
        list_role = Role.query.filter_by(
                        project_id=i.id).all()
        for j in list_role:
            list_user_role = UserRole.query.filter_by(
                                role_id=j.id).all()
            ...
            db.session.flush()
        # xóa reference
        list_reference = References.query.filter_by(
                            project_id=i.id).all()
        for j in list_reference:
            db.session.delete(j)
            db.session.flush()
        # xóa issue
        list_issue = Issue.query.filter_by(
                        project_id=i.id).all()
        for j in list_issue:
            # xóa issue_label
            list_issue_label = IssueLabel.query.filter_by(
                                issue_id=j.id).all()
            for k in list_issue_label:
                db.session.delete(k)
                db.session.flush()
            # xóa resources
            list_resource = Resources.query.filter_by(
                                issue_id=j.id).all()
            for k in list_resource:
                db.session.delete(k)
                db.session.flush()
            # xóa activity
            list_activity = Activity.query.filter_by(
                                issue_id=j.id).all()
